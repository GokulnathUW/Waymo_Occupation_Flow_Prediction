...
def visualize_all_agents_smooth(
  ...
  """
  # [num_agents, num_past_steps, 2] float32. (num_past_steps = 10 as there are 10 frames per second)
  past_states = tf.stack(
      [decoded_example['state/past/x'], decoded_example['state/past/y']], -1 # Previous x,y coordinates
  ).numpy()
  past_states_mask = decoded_example['state/past/valid'].numpy() > 0.0

  # [num_agents, 1, 2] float32.
  current_states = tf.stack(
      [decoded_example['state/current/x'], decoded_example['state/current/y']], # Current x,y coordinate
      -1,
  ).numpy()
  ...
  future_states = tf.stack(
      [decoded_example['state/future/x'], decoded_example['state/future/y']], -1 # Future x,y coordinates
  ).numpy()
  ...
  for i, (s, m) in enumerate(
    ...
    im = visualize_one_step(
        s[:, 0], # x,y coordinates of all agent's state in that step
        m[:, 0],
        roadgraph_xyz,
